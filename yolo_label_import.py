...
"""
This script converts CVAT XML annotations (with Onion/Weed boxes) from a ZIP to the YOLO format
...
- Extracts the zip to a folder next to the zip (same name without .zip),
  ...
  (class 1), converts coordinates to YOLO normalised format, shuffles and
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Convert CVAT XML (with Onion/Weed boxes) from a ZIP → YOLO format "
    ...
    for img_elem in root.findall(".//image"):
        ...
        for box in img_elem.findall("box"):
            ...
            if label not in name_to_id:
                continue  # skip unwanted classes (Spinach, Uncertain, ...)
...
def write_data_yaml(out_dir: Path) -> None:
    ...
    Assumes two classes: onion (0) and weed (1).
    """
